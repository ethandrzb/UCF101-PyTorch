...
model = STAM(
    dim = 512,
    image_size = 112,     # size of image
    patch_size = 16,      # patch size
    num_frames = clip_len,       # number of image frames, selected out of video
...
frames = torch.randn(2, clip_len, 3, 112, 112).to(device) # (batch x frames x channels x height x width)
